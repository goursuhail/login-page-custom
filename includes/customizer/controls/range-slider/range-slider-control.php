	...
	function o2_add_range_slider_control( $wp_customize ) {
		class O2_Customizer_Range_Slider_Control extends WP_Customize_Control {
			...
			public function enqueue() {
				wp_enqueue_script( 'o2-range-slider', CLP_URL.'/includes/customizer/controls/range-slider/assets/js/range-slider-control.js', array( 'jquery' ), '', true );
				wp_enqueue_style( 'o2-range-slider', CLP_URL.'/includes/customizer/controls/range-slider/assets/css/range-slider-control.css' );
			}
					...
					<?php if ( ! empty( $this->label ) ) : ?>
						...
						<?php if ( isset( $this->choices['percent'] ) && ( $this->choices['percent'] !== false ) ) : ?>
							<div class="button-group" data-setting="align">
								...
								<label class="button" for="px-<?php echo esc_attr( $this->id ); ?>"><?php _e( 'px', 'custom-login-page' ); ?></label>
								...
								<label class="button" for="percent-<?php echo esc_attr( $this->id ); ?>"><?php _e( '%', 'custom-login-page' ); ?></label>
							</div>
						<?php endif; ?>
						</span>
						<?php
